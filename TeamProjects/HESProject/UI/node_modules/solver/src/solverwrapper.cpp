...
void Solver::GetTable(const Arguments& args) {
	...
	double* table = new double[(mGridN + 1) * (mGridM + 1)];
	...
	Local<Array> rows = Array::New(isolate, mGridM + 1);
	for (int i = 0; i <= mGridM; i++) {
		Local<Array> row = Array::New(isolate, mGridN + 1);
		rows->Set(i, row);
		for (int j = 0; j <= mGridN; j++) {
			double tij = table[mGridM * i + j];
