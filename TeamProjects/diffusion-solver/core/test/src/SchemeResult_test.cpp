...
SchemeSolverResult RandSchemeSolverResult(int testSize, int seed1, int seed2) {
   ...
   SchemeSolverResult res(solutionU1, solutionU2, 
                    testSize - 1, 1, 0.001);
   return res;
}

SchemeSolverResult FillSchemeSolverResult(int testSize, double val1, double val2) {
   auto solutionU1 = MakeSharedArray(testSize);
...
void CompareSchemeSolverResult(SchemeSolverResult& res, double val1, double val2) {
...
TEST(SchemeSolverResult, Initialization) {
   for (int i = 10; i < 100; ++i) {
      RandSchemeSolverResult(i, 0, 1);
   }
}

TEST(SchemeSolverResult, DataAccessing) {
   double val1 = 1.5;
   double val2 = 2.5;
   int length = 1000;
   for (int i = 0; i < 50; ++i) {
      auto res = FillSchemeSolverResult(length, val1 * i, val2 * i);
      CompareSchemeSolverResult(res, val1 * i, val2 * i);
   }
}

TEST(SchemeSolverResult, MinMax) {
   double targetVal = 100;
   auto result = FillSchemeSolverResult(100, targetVal, targetVal);
...
TEST(SchemeSolverResult, Copying) {
   double targetVal = 100;
   auto result = FillSchemeSolverResult(100, targetVal, targetVal);
   auto resultPtr = new SchemeSolverResult(result);
   CompareSchemeSolverResult(*resultPtr, targetVal, targetVal);
   delete resultPtr;
}
